...
document.getElementById("total").insertAdjacentText("beforeend", users.length);


let newUsers = users.map( user => render( user ) );
function render (input) {
    let email=input.name+"@example.com";
    let  tab = ` <li class="contact-item cf">
    <div class="contact-details">
            <img class="avatar" src=${input.image}>
            <h3>${input.name}</h3>
            <span class="email">${email.split(" ").join("")}</span>
    </div>
    <div class="joined-details">
            <span class="date">${input.joined}</span>
    </div> 
    </li>
    `;
    document.getElementById("new").insertAdjacentHTML("beforeend",tab);
